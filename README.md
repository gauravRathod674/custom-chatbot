...
Required. Your Google Gemini API key. If not provided, the bot will simply echo user messages.
...
The initial message displayed by the bot when it opens.
...
Controls whether the chat window is open or closed initially.
...
A theme object to customize the UI. See the Theming Options table below.
...
For advanced use. A controlled array of message objects.
...
For advanced use. Manually controls the typing indicator for the bot.
...
Color of the icon in the launcher.
...
Background color of the chat window header.
...
Shape of message bubbles ('rounded' or 'square').
...
Color of the focus ring on the input field.
